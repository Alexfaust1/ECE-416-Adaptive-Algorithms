%% Alexander Faust
%
% ECE 416 - Question 5 Part B
%
% 
clc; clear; close all;
%% Question 5 - Part B
% In this script I will be simply changing the number of sensors and 
% and snapshots then reporting only the crucial analyses I found important 
% since there is just way too much to discuss if I look into detail for 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% WE HAVE TWEAKED THIS PARAMETER ALSO
% Generate sensor locations on the x and y axes in cross formation:
r = d*[(-5:1:5) zeros(1, 10); zeros(1, 11) (-5:1:-1) (1:1:5); zeros(1, 21)];
...
%% Concluding remarks
% Okay so now after changing the number of snapshots to much lower and
% decreasing the number of sensors, I gained some more insight on the
% effects here. So overall, the algorithms performed horribly, even when
% using the optimal algorithm parameters used in the original exploration
% of questions 2-4. Especially was the case for MVDR where the results were
% almost laughable. It was so bad for MVDR that in some cases the
% beamformer was attenuating all 3 sources equally, and amplifying other
% AOA's! It should be obvious why this is a problem... (it's no longer easy
% to distinguish between the distinct sources when the beamformer is
% steered to any particular source.
%
% However, what was interesting is that from a graphical perspective, the
% learning curves for the algorithms improved quite substantially, and that
% de-converging phenomenon for RLS as seen in experiments 2-4 (in the
% original script) was rid of in these tests. Somehow, less information
% (through the acquisition of more snapshots) improved the convergence of
% the learning curve to J_min. It appears as if this is the tradeoff. 
%
% Perhaps at greater snapshots the algorithm becomes more "correlated" with
% previous weights due to the precision and starts performing poorly for
% some multi-dimensional reason that does not seem obvious. However, is
% this tradeoff worth it compared to just using more sensors and more
% snapshots? - Probably not. But, it would be interesting to see if there
% become diminishing returns after a certain number of snapshots taken with
% 'x' number of sensors used.
%
% I am glad that I saw some appreciable change for these experiments
% because now my intuition seems to have grown some more for this type of
% situation.
